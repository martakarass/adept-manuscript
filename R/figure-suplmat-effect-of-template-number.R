...
res.df <- as.data.frame(fread(file.path("data-results", "suplmat-effect-of-template-number.csv")))
...
plt <- 
  ggplot(plt.df, aes(x = factor(template_n), y = T_i/100)) + 
  geom_boxplot(outlier.size = 0.7) + 
  facet_grid(subj_id ~ loc_id) + 
  theme_Publication() + 
  labs(x = "Number of distinct stride pattern functions used",
