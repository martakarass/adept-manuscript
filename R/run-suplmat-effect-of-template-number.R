...
for (subj_id.tmp in subj_id.unique.vec){
  for (loc_id.tmp in loc_id.unique.vec){
    ...
    for (template_n.tmp in 1:5){
  
      ## Construct list of empirical pattern templates 
      template.tmp.x <- t(stride_template[[loc_id.tmp]][[template_n.tmp]])
      ...
      out.tmp$template_n <- template_n.tmp
...
write.table(res.df,
            file.path("data-results", "suplmat-effect-of-template-number.csv"),
